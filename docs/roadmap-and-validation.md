...
1. ~~Replace the single shared bearer-secret posture with a clearer operator model and a staged path toward stronger user or role scoping if the standalone remains server-side.~~ **Done (R-03)**: JWT Bearer auth now supports HS256 shared-secret verification, RS256 single-key public-key verification, or local kid-aware JWKS verification, plus principal extraction and role-based `RequestPrincipal`. API key auth is preserved for backward compatibility, while `JWT_SECRET`, `JWT_PUBLIC_KEY`, and `JWT_JWKS` remain mutually exclusive JWT modes.
	Follow-through: owner-scoped case visibility, revocable sharing, and local kid-aware JWKS rollover are now implemented for JWT principals, while remote issuer-bound JWKS retrieval and richer RBAC remain backlog.
...
4. Add remote JWKS retrieval and cache control, richer sharing policy, or tenant-scoped RBAC only if the standalone needs collaboration beyond the current revocable owner-admin sharing model.
