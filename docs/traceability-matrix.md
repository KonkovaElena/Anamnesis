...
| Secure-by-default bearer auth startup | Normal runtime requires either `API_KEY`, `JWT_SECRET`, `JWT_PUBLIC_KEY`, or `JWT_JWKS`; unauthenticated startup requires an explicit dev override and is blocked in production mode. The JWT path validates `sub`, `iss`, `aud`, `nbf`, `iat`, `exp`, malformed `roles`, optional JOSE `typ`, supports HS256, RS256 single-key verification, or local `kid`-aware JWKS verification, rejects weak HS256 secrets, and rejects RSA verification keys below 2048 bits. | `src/index.ts`, `src/bootstrap.ts`, `src/application/auth-middleware.ts`, `src/core/jwt-verification.ts` | `tests/auth.test.ts`, `tests/jwt-auth.test.ts` |
