...
| Secure-by-default bearer auth startup | Normal runtime requires either `API_KEY`, `JWT_SECRET`, `JWT_PUBLIC_KEY`, `JWT_JWKS`, or `JWT_JWKS_URL`; unauthenticated startup requires an explicit dev override and is blocked in production mode. The JWT path validates `sub`, `iss`, `aud`, `nbf`, `iat`, `exp`, malformed `roles`, optional JOSE `typ`, supports HS256, RS256 single-key verification, local `kid`-aware JWKS verification, or issuer-bound remote JWKS retrieval with cache-aware refresh, rejects weak HS256 secrets, and rejects RSA verification keys below 2048 bits. | `src/index.ts`, `src/bootstrap.ts`, `src/application/auth-middleware.ts`, `src/core/jwt-verification.ts`, `src/infrastructure/RemoteJwtJwksProvider.ts` | `tests/auth.test.ts`, `tests/jwt-auth.test.ts`, `tests/remote-jwks.test.ts` |
