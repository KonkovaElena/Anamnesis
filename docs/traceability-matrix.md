...
| Secure-by-default bearer auth startup | Normal runtime requires either `API_KEY`, `JWT_SECRET`, or `JWT_PUBLIC_KEY`; unauthenticated startup requires an explicit dev override and is blocked in production mode. The JWT path validates `sub`, `iss`, `aud`, `nbf`, `iat`, `exp`, malformed `roles`, optional JOSE `typ`, supports HS256 or RS256 verification, rejects weak HS256 secrets, and rejects RSA verification keys below 2048 bits. | `src/index.ts`, `src/bootstrap.ts`, `src/application/auth-middleware.ts`, `src/core/jwt-verification.ts` | `tests/auth.test.ts`, `tests/jwt-auth.test.ts` |
...
| Grant-only delegated case sharing | A JWT case owner or an API-key operator can add another JWT principal to the case access list, shared principals gain case-bound workflow access, non-owner shared principals cannot re-grant access, and deleted-case audit history remains readable to principals who held access before deletion. | `src/domain/anamnesis/access-control.ts`, `src/domain/anamnesis/case-workflow.ts`, `src/application/create-app/case-routes.ts`, `src/core/audit-events.ts` | `tests/case-sharing.test.ts` |
...
| LLM sidecar contract readiness | The system defines a domain port and a no-op adapter for future local draft-assistance sidecars without claiming active model-generated packet content today. | `src/domain/anamnesis/interfaces.ts`, `src/infrastructure/NoOpLlmSidecar.ts` | `tests/llm-sidecar.test.ts` |
