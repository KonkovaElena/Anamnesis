...
| Optional bounded LLM draft assistance | When a local sidecar is configured and available, packet drafting can append bounded draft-assistance sections and flat audit metadata while preserving deterministic fail-closed drafting when the sidecar is absent or errors. Operator-visible request, success, failure, and timestamp telemetry is exposed through the existing ops summary and metrics surfaces. This remains draft-only clinician support, not autonomous diagnosis or treatment. | `src/domain/anamnesis/interfaces.ts`, `src/infrastructure/NoOpLlmSidecar.ts`, `src/infrastructure/OpenAiCompatibleLlmSidecar.ts`, `src/bootstrap.ts`, `src/index.ts`, `src/application/create-app/packet-routes.ts`, `src/application/create-app/ops-routes.ts`, `src/application/create-app/shared.ts` | `tests/llm-sidecar.test.ts`, `tests/llm-packet-enrichment.test.ts` |
