# Documentation Map

This page routes the active documentation surfaces for Anamnesis.

## Product And Architecture

- [../design.md](../design.md)
- [../openapi.yaml](../openapi.yaml)
- [architecture/overview.md](architecture/overview.md)
- [api-scope.md](api-scope.md)
- [interop/README.md](interop/README.md)
- [status-model.md](status-model.md)

## Safety, Scope, And Roadmap

- [claim-boundary.md](claim-boundary.md)
- [roadmap-and-validation.md](roadmap-and-validation.md)
- [security/posture-and-gaps.md](security/posture-and-gaps.md)

## Evidence And Research

- [academic/evidence-register.md](academic/evidence-register.md)
- [traceability-matrix.md](traceability-matrix.md)

## Investor And Publication Pack

- [investor/README.md](investor/README.md)
- [../SUPPORT.md](../SUPPORT.md)
- [../GOVERNANCE.md](../GOVERNANCE.md)
- [../PUBLISHING.md](../PUBLISHING.md)

## Historical Design Notes

- [design/standalone-extraction.md](design/standalone-extraction.md) remains historical context for the repository split and should not be treated as the current public product narrative.
