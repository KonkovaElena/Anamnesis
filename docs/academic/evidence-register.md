...
### Current Local Evaluation Gate

| Surface | Current local evidence | Operational consequence for Anamnesis |
|---|---|---|
| OpenAI-compatible `LlmSidecar` draft assistance | Fixture-backed baseline evaluation pack now exists in `tests/llm-sidecar-evaluation.test.ts` with accepted review-oriented fixture cases plus rejected unsafe clinical fixture cases sourced from `tests/fixtures.ts`. The current baseline covers chronology-and-gap summaries, review-question framing, diagnosis language, treatment recommendations, and prescription language. The named local rail for this pack is `npm run test:llm-evaluation`. | The current sidecar seam now has a concrete local go/no-go rule: all accepted fixtures must enrich successfully, all rejected fixtures must fail closed without packet enrichment, and the packet workflow must remain deterministic when the sidecar output is unsafe. This is sufficient for the current draft-only seam, but not yet for widening public claims or enabling retrieval-backed assistance. |
...
| `arXiv:2602.21368` — Black-Box Reliability Certification | Reliability should be treated as an explicit deployment gate derived from sampling and calibration, not informal confidence. | The current sidecar seam now has a baseline local go/no-go gate through the fixture-backed evaluation pack. No future AI-generated workflow feature should be promoted beyond the current draft-only scope without a broader fixed evaluation pack and an explicit upgraded go/no-go gate. |
