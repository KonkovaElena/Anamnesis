...
| `/api/cases/{caseId}/physician-packets` | `POST` | Draft a physician packet from the current case state. Under JWT, `requestedBy` is bound to the authenticated `sub`. When a local `LlmSidecar` is configured and available, including the OpenAI-compatible runtime path enabled through `LLM_SIDECAR_BASE_URL` plus `LLM_SIDECAR_MODEL`, the route may append bounded draft-assistance sections and flat enrichment metadata in the corresponding `packet.drafted` audit event; if the sidecar is absent or errors, drafting remains deterministic and succeeds without model output. |
