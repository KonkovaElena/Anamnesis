...
- **R-03 JWT Bearer authentication**: JWT verification in `src/core/jwt-verification.ts` now supports HS256 shared-secret validation, RS256 single-key public-key validation, and kid-aware local JWKS validation (zero external dependencies); auth middleware supports both API key and JWT Bearer tokens with principal extraction; `JWT_SECRET`, `JWT_PUBLIC_KEY`, `JWT_JWKS`, `JWT_ISSUER`, and `JWT_AUDIENCE` env vars; `request.principal` populated with `RequestPrincipal` including roles and claims; dedicated JWT unit and integration coverage in `tests/jwt-auth.test.ts` and `tests/auth.test.ts`;
- **R-02 bounded LLM draft assistance seam**: `LlmSidecar`, `LlmDraftAssistanceInput`, and `LlmDraftAssistanceResult` interfaces now wire through bootstrap and packet drafting as an optional fail-closed application-layer enrichment path; `NoOpLlmSidecar` remains the default adapter, available sidecars can append bounded draft-assistance sections and flat audit metadata to `packet.drafted`, and focused contract plus API coverage now lives in `tests/llm-sidecar.test.ts` and `tests/llm-packet-enrichment.test.ts`;
